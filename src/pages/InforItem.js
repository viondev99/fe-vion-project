...
export function InforItem() {
  const auth = useAuth()
  const { id } = useParams()
  console.log('aaaaaa', useParams())
  const productCtx = useProductCtx()

  useEffect(() => {
    const newId = id.split('-')
    productCtx.getProductById(newId[0])
          ...
          <div className="flex mt-16 mb-16" id="form-img">
            <div className="flex-1 border-r-2">
              <div className="border-2 border-orange-200 pt-10 pb-10 w-10/12 ml-auto mr-auto">
                <img
                  src={productCtx?.selectedItem?.photos}
                  alt=""
                  width="400"
                  className="block ml-auto mr-auto h-full"
                />
              </div>
            </div>
              ...
              <div className="w-10/12 ml-auto mr-auto">
                ...
                <div className="mt-5 border-b-slate-600 border-b-2">
                  ...
                  <div className="w-full h-72 border-2 mb-5">
                    <p className="break-words m-3">
                      {productCtx?.selectedItem?.description}
                    </p>
                  </div>
                </div>
                <div className="flex mt-5 text-2xl">
                  <div className="font-bold">
                    <p>
                      Giá:{' '}
                      {Number(productCtx?.selectedItem?.price).toLocaleString()}{' '}
                      ₫
                    </p>
                  </div>
                </div>
                ...
                <div className="mt-7">
                  {auth.user?.access_token ? (
                    ...
                    >
                      <div className="flex">
                        <img
                          src={Cart}
                          alt=""
                          className="h-8 block ml-auto mr-auto"
                        />
                    ...
                    <button className="border-2 border-orange-600 h-12 w-6/12 active:bg-white active:text-orange-600 font-bold bg-orange-600 text-white text-xl">
                      <Link to="/signin">
                        <div className="flex">
                          <img
                            src={Cart}
